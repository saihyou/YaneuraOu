	...
	**/
	...
	if (   !ss->ttPv
		...
		&&  eval - futility_margin(depth, cutNode && !ss->ttHit, improving)
				- (ss - 1)->statScore / 327 >= beta
		&&  eval >= beta
		&&  eval < 27734 // smaller than TB wins
		&& (!ttMove || ttCapture))
	...
	// 3秒以上経過しているなら現在探索している指し手をGUIに出力する。
		...
		if (ss->ply < thisThread->rootDepth * 2)
		{
			...
			if (!rootNode
		        ...
		        &&  depth >= PARAM_SINGULAR_EXTENSION_DEPTH - (thisThread->completedDepth > 31) + ss->ttPv
			...
			// PV nodeでquietなttは良い指し手のはずだから延長するというもの。
				...
				&& move == ttMove
				&& move == ss->killers[0]
				&& (*contHist[0])(movedPiece, to_sq(move)) >= 4111)
				extension = 1;
		...
		if (value > bestValue)
		{
			...
			if (value > alpha)
			{
				...
				else
				{
					...
					if (   depth > 2
						&& depth < 12
						&& beta  <  13195 /* VALUE_TB_WIN_IN_MAX_PLY */
						&& value > -12346 /* VALUE_TB_LOSS_IN_MAX_PLY*/)
